 ...
 * (QHash<screenName, QSet<windowId>>) plus a reverse index
 * (windowId -> screenName) for O(1) lookups.
 *
...
public:
     ...
     * No-op if windowId or screenName is empty.
     */
     ...
     * @param screenName Screen where overflow is being applied
     ...
     *
     * @param screenName Screen to check for recovery
     ...
     */
    QStringList recoverIfRoom(const QString& screenName, int tiledCount, int maxWindows,
     ...
     */
    QSet<QString> takeForScreen(const QString& screenName);
